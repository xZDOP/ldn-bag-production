...
pBP = Tunnel.getInterface("ldn-bag-production")
...
local serviceX = 997.78
local serviceY = -2109.08
local serviceZ = 30.48

-- 997.78,-2109.08,30.48
	...
	while true do
		...
		if not inService then
			...
			if distance <= 10.1 then
				idle = 5
				DrawMarker(21,serviceX,serviceY,serviceZ-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,255,140,0,50,0,0,0,1)
				if distance <= 3.1 then
					if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), serviceX,serviceY,serviceZ, true ) <= 3.1  then
						DrawText3D(serviceX,serviceY,serviceZ, "PRESSIONE ~o~E~w~ PARA INICIAR A ~o~COLETA~w~ DE ~o~MOCHILAS~w~")
					end
					if IsControlJustPressed(0,38) and not inService then
						if pBP.checkPermission() then
							inService = true
							check = math.random(#deliverys)
							makeBlipsServices(deliverys,check)
							TriggerEvent("Notify","sucesso","Uma <b>coleta</b> de mochilas foi <b>encontrada</b>!",8000)
			...
			if distance <= 10.1 then
				...
				if distance <= 3.1 then
					if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), deliverys[check].x,deliverys[check].y,deliverys[check].z, true ) <= 3.1  then
						DrawText3D(deliverys[check].x,deliverys[check].y,deliverys[check].z, "PRESSIONE ~o~E~w~ PARA COLETAR AS ~o~MOCHILAS~w~")
					end
...
Citizen.CreateThread(function()
    ...
    if inService and IsControlJustPressed(0,121) then
      ...
      elseif IsControlJustPressed(0,168) then
        inService = false
        RemoveBlip(blips)
        TriggerEvent("Notify","aviso","Você <b>finalizou</b> a coleta de <b>mochilas</b>.")
		end
	end
end)
